...
static inline void pin_to_core(size_t core)
{
    ...
    if (sched_setaffinity(0, sizeof(cpu_set_t), &cpuset) == 0)
        return;
    /* The requested core may be outside the caller's cpuset (device shell)
     * or hotplugged off (EINVAL).  Fall back into the allowed mask: the
     * (core % nallowed)'th allowed CPU, so the exp32 waiter/consumer keep
     * distinct cores whenever the mask has more than one.  Never fatal —
     * an unpinned thread is racier but still correct. */
    cpu_set_t allowed;
    CPU_ZERO(&allowed);
    if (sched_getaffinity(0, sizeof(allowed), &allowed) == 0) {
        size_t n = CPU_COUNT(&allowed);
        if (n > 0) {
            size_t want = core % n, seen = 0;
            for (size_t c = 0; c < CPU_SETSIZE; c++) {
                ...
                if (seen++ == want) {
                    ...
                    CPU_SET(c, &cpuset);
                    if (sched_setaffinity(0, sizeof(cpu_set_t), &cpuset) == 0)
                        pr_warning("pin_to_core: core %zu unavailable, using %zu\n",
                                   core, c);
                    else
                        pr_warning("pin_to_core: fallback to %zu failed errno=%d\n",
                                   c, errno);
                    return;
                }
            }
    ...
    pr_warning("pin_to_core: core %zu unavailable, unpinned (errno=%d)\n",
               core, errno);
}
